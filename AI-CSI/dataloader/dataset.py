...
class CSIDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for label in self.labels:
            ...
            sample[label]['support'] = torch.FloatTensor(np.array(support_set)).to(self.device)
            ...
            sample[label]['query'] = torch.FloatTensor(np.array(query_set)).to(self.device)
...
class FSLDataset(data.Dataset) :
    def __init__(self, data_path, win_size=10, mode='train', amp=True, mac=True, time=True):
        ...
        self.data_x, self.data_y = self.generate_windows()
        self.data_x = torch.FloatTensor(self.data_x).to(self.device)
        self.data_y = torch.LongTensor(self.data_y).to(self.device)
...
if __name__ == '__main__':
    ...
    print(f"Number of episodes: {data.__len__()}")
    sample = data.__getitem__(0)
    print(f"Support set shape: {sample['empty']['support'].shape}")
    print(f"Support set device: {sample['empty']['support'].device}")